...
class MyText:
    ...
    def t_answer(self,t):
        ...
        self.answer_count += 1
        self.answer_type = 1    # 0: numerical answers  1: formula answers

    def t_answer_numerical(self,t):
        r'Compute\(\"|\"'
        self.answer_type = 0    # 0: numerical answers  1: formula answers
        t.lexer.code_start = t.lexer.lexpos
        t.lexer.begin('numerical')

    def t_numerical_element(self,t):
        r'[^\"]'

    def t_numerical_end(self,t):
        r"\"?\)?\}\.?"
        value = t.lexer.lexdata[t.lexer.code_start:t.lexer.lexpos-len(t.value)]
        self.answers = self.answers + 'answer' + str(self.answer_count) + ' = 1.0*' + value.replace('^','**') + '\n'
        t.type = "NUMERICAL_ANSWER"
        t.lexer.begin('INITIAL')
        return t

    def t_answer_formula(self,t):
        r'Formula\(\"'
        self.answer_type = 1
        t.lexer.code_start = t.lexer.lexpos
        t.lexer.begin('formula')

    def t_formula_element(self,t):
        r'[^\"]'

    def t_formula_end(self,t):
        r"\"?\)?\}\.?"
        value = t.lexer.lexdata[t.lexer.code_start:t.lexer.lexpos-len(t.value)]
        self.answers = self.answers + 'answer' + str(self.answer_count) + ' = "' + value.replace('**','^') + '"' + '\n'
        t.type = "FORMULA_ANSWER"
        t.lexer.begin('INITIAL')
        return t
    ...
    def t_answer_end(self,t):
        ...
        if self.answer_type == 0:
            ...
            self.answers = self.answers + 'answer' + str(self.answer_count) + ' = "' + value + '"' + '\n'
            t.type = "FORMULA_ANSWER"
        t.lexer.begin('INITIAL')
        return t
    ...
    def p_newline(self,p):
        ...
        '''
        #else:
        p[0] = '<br/>\n'          # When we are in the middle of a math expression, using <p></p> will destroy the form 
    ...
    def p_f_ansewr(self,p):
        '''
        answer : FORMULA_ANSWER
        '''
        self.answer_blank_count += 1 # To record the number of answer blanks and give each answer blank a distint ID
                                     # in order to match the answer ID which will appear in the code part before the text part  
        s = '''<formularesponse type="ci" samples="{Sample}" answer="$answer'''+str(self.answer_blank_count)+'''">
